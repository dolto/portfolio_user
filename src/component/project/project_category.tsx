...
export default async function Project_category(props: Prop) {
  ...
  const skillDB = await db
    .collection<SkillCollection>("Skill")
    .find()
    .toArray();
  const langs_slecets: string[] = props.langs_slecets;
  const skills_slects: string[] = props.skills_slects;
  const langs =
    langs_slecets.length == 0
      ? langsDB
      : langsDB.filter((db) => langs_slecets.includes(db._id.toString()));
  const skills =
    skills_slects.length == 0
      ? skillDB
      : skillDB.filter((db) => skills_slects.includes(db._id.toString()));
  return (
    <article className={"project_category_selecter"}>
      ...
      <nav className={"project_langs"}>
        {langsDB.map((lang) => {
          return (
            <Link
              href={`?is_blog=${!props.is_Blog}&langs_slecets=${JSON.stringify(
                langs_slecets.length == langsDB.length
                  ? [lang._id.toString()]
                  : langs_slecets.includes(lang._id.toString())
                  ? langs_slecets.filter((s) => !(lang._id.toString() == s))
                  : langs_slecets.concat(lang._id.toString())
      ...
      <nav className={"project_skills"}>
        {skillDB.map((skill) => (
          <Link
            href={`?is_blog=${!props.is_Blog}&langs_slecets=${JSON.stringify(
              langs_slecets
            )}&skills_slects=${JSON.stringify(
              skills_slects.length == skillDB.length
                ? [skill._id.toString()]
                : skills_slects.includes(skill._id.toString())
                ? skills_slects.filter((s) => !(skill._id.toString() == s))
                : skills_slects.concat(skill._id.toString())
            )}`}
